...
import { FileText, Sparkles, CheckCircle, AlertCircle, Loader2, MessageSquare } from "lucide-react"
...
export default function ApplicationDetailPage() {
  ...
  useEffect(() => {
    if (!sessionLoading && (!user || user.role !== "student")) {
  ...
  const handleGenerateResume = async () => {
    ...
    try {
      const newResume = await demoApi.generateTailoredResume(application.id)
      setResume(newResume)
      toast({
        title: "Resume Generated!",
        description: "Your tailored resume is ready to review",
      })
    } finally {
  ...
  const handleGenerateCoverLetter = async () => {
    setIsGenerating(true)
    try {
      const newCoverLetter = await demoApi.generateCoverLetter(application.id)
      setCoverLetter(newCoverLetter)
      toast({
        title: "Cover Letter Generated!",
        description: "Your cover letter is ready to review",
      })
    } finally {
      setIsGenerating(false)
    }
  ...
  const handleQualityCheck = async () => {
    setIsChecking(true)
    try {
      const check = await demoApi.runQualityCheck(application.id)
      ...
      <div className="space-y-6 animate-fade-in">
        ...
        <div className="flex flex-col sm:flex-row items-start justify-between gap-4">
          <div>
            <div className="flex flex-wrap items-center gap-3 mb-2">
              ...
              <Badge className={statusColors[application.status]}>{application.status.replace(/_/g, " ")}</Badge>
            </div>
          ...
          {application.readiness_score !== null && (
            <div className="text-left sm:text-right">
              <div className="text-3xl sm:text-4xl font-bold text-primary mb-1">{application.readiness_score}</div>
        ...
        <Tabs defaultValue="overview" className="space-y-6">
          ...
          <TabsContent value="overview" className="space-y-6">
            ...
            <Card className="p-4 sm:p-6 glass">
              <h3 className="text-lg sm:text-xl font-semibold mb-4">Application Progress</h3>
              <div className="space-y-4">
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                  ...
                  {!resume && (
                    <Button onClick={handleGenerateResume} disabled={isGenerating} size="sm">
                      {isGenerating ? (
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                  ...
                  {!coverLetter && (
                    <Button onClick={handleGenerateCoverLetter} disabled={isGenerating} size="sm">
                      {isGenerating ? (
                        <Loader2 className="size-4 animate-spin mr-2" />
                      ) : (
                        <Sparkles className="size-4 mr-2" />
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                  ...
                  {resume && coverLetter && !qualityCheck && (
                    <Button onClick={handleQualityCheck} disabled={isChecking} size="sm">
                      {isChecking ? (
                        <Loader2 className="size-4 animate-spin mr-2" />
                      ) : (
            ...
            {resume ? (
                    ...
                    {resume.structured_json.education?.map((edu: any, i: number) => (
                      <div key={i}>
                        ...
                        {edu.relevant_coursework && (
                          <p className="text-sm mt-1">Relevant Coursework: {edu.relevant_coursework}</p>
                        )}
                      </div>
                    ))}
                  ...
                  <Card className="p-4 sm:p-6 glass border-secondary/50">
                    ...
                    {feedback.map((fb) => (
                      <div key={fb.id} className="bg-secondary/10 p-4 rounded-lg">
                        ...
                        <p className="text-sm text-muted-foreground mt-2">{new Date(fb.created_at).toLocaleString()}</p>
                      </div>
                    ))}
                  </Card>
                )}
              ...
              <Card className="p-8 sm:p-12 text-center glass">
                <div className="max-w-md mx-auto space-y-4">
                  ...
                  <h3 className="text-lg sm:text-xl font-semibold">No Resume Generated Yet</h3>
                  <p className="text-muted-foreground text-sm sm:text-base">
                    Generate an AI-tailored resume for this position
                  </p>
                  <Button
          ...
          <TabsContent value="cover-letter" className="space-y-6">
            {coverLetter ? (
              ...
              <Card className="p-8 sm:p-12 text-center glass">
                <div className="max-w-md mx-auto space-y-4">
                  <FileText className="size-12 sm:size-16 text-muted-foreground mx-auto" />
                  <h3 className="text-lg sm:text-xl font-semibold">No Cover Letter Generated Yet</h3>
                  <p className="text-muted-foreground text-sm sm:text-base">
                    Generate an AI-tailored cover letter for this position
                  </p>
                  <Button
          ...
          <TabsContent value="quality" className="space-y-6">
            {qualityCheck ? (
              <>
                <Card className="p-4 sm:p-6 glass">
                  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
                    ...
                    <div className="text-2xl sm:text-3xl font-bold text-primary">{application.readiness_score}</div>
                  </div>
              ...
              <Card className="p-8 sm:p-12 text-center glass">
                <div className="max-w-md mx-auto space-y-4">
                  <CheckCircle className="size-12 sm:size-16 text-muted-foreground mx-auto" />
                  <h3 className="text-lg sm:text-xl font-semibold">Quality Check Not Run</h3>
                  <p className="text-muted-foreground text-sm sm:text-base">
                    {!resume || !coverLetter
                      ? "Generate both resume and cover letter before running quality check"
                      : "Run quality check to assess your application readiness"}
                  </p>
                  {resume && coverLetter && (
                    <Button
